<x-admin-layout>
                        ...
                        <table class="table table-bordered table-striped table-hover shop-cart">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label class="toggle-switch">
                                            <input type="checkbox" data-id="{{ $advertisement->id }}" class="sample-toggle" {{ $advertisement->sample == 1 ? 'checked' : '' }}>
                                            <span class="slider"></span>
                                        </label>   
                                    </td>
                                    @endif
    ...
    <script>
        $(document).ready(function() {
            // Listen for the toggle switch change event
            $('.sample-toggle').on('change', function() {
                // Get the section ID from the data-id attribute
                var sectionId = $(this).data('id');
                // Determine the status based on whether the checkbox is checked
                var status = $(this).is(':checked') ? 1 : 0;
                ...
                // Send AJAX request to update the section status
                $.ajax({
                    url: '/admin/update/advertisement-sample', // The route URL
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}', // CSRF token for security
                        id: sectionId,
