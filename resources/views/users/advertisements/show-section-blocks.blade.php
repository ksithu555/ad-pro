<x-user-layout>
    ...
    <section class="white-bg pt-120 pb-120">
      <div class="container">
        @php
            ...
            if ($type == 'countdown') {
                $addRoute = 'user.add.countdown.block';
                $editRoute = 'user.edit.countdown.block';
                $deleteRoute = 'user.delete.countdown.block';
            }
        @endphp
        <div class="row">
          <div class="col-md-2">
                    ...
                    <table class="table table-bordered table-striped table-hover shop-cart">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="min-width: 110px;">
                                    {{ \Carbon\Carbon::parse($block->target_datetime)->format('Y-m-d H:i') }}
                                </td>
                                <td style="min-width: 110px;">
                                  <label class="toggle-switch">
                                      <input type="checkbox" data-id="{{ $block->id }}" class="status-toggle-countdown" {{ $block->status == 1 ? 'checked' : '' }}>
                                      <span class="slider"></span>
                                  </label>   
                                </td>
                                <td>
    ...
    <script>
        $(document).ready(function() {
            // Listen for the toggle switch change event
            $('.status-toggle-video').on('change', function() {
                // Get the section ID from the data-id attribute
                var blockId = $(this).data('id');
                // Determine the status based on whether the checkbox is checked
                var status = $(this).is(':checked') ? 1 : 0;
    ...
    <script>
        $(document).ready(function() {
            // Listen for the toggle switch change event
            $('.status-toggle-countdown').on('change', function() {
                // Get the section ID from the data-id attribute
                var blockId = $(this).data('id');
                // Determine the status based on whether the checkbox is checked
                var status = $(this).is(':checked') ? 1 : 0;
                ...
                // Send AJAX request to update the section status
                $.ajax({
                    url: '/user/update/section/countdown-block-status', // The route URL
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}', // CSRF token for security
                        id: blockId,
