<x-user-layout>
    ...
    <section class="white-bg pt-120 pb-120">
        <div class="container">
            <div class="row">
                ...
                <h4 class="text-uppercase mt-0">サブ画像ブロック修正</h4>
                </div>
            </div>
            <div class="row mt-50">
                <div class="col-md-12">
                    <form name="edit-sub-image-block-form" id="edit-sub-image-block-form" action="{{ route('user.update.sub.image.block') }}" method="POST" 
                        class="contact-form-style-01" enctype="multipart/form-data">
                        @csrf
                        <div class="messages"></div>
                        <input type="hidden" id="id" name="id" value="{{ $subImageBlock->id }}">
                        <div class="row">
                            <div class="col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2">
                                <div class="form-group">
                                    <label class="sr-only" for="title">タイトル</label>
                                    <input type="text" name="title" class="md-input" id="title" placeholder="タイトル *" value="{{ old('title') ? old('title') : $subImageBlock->title }}">
                        ...
                        <div class="row">
                            <div class="col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2">
                                <div class="form-group">
                                    ...
                                    <textarea name="body" class="md-textarea" id="body" rows="7" placeholder="内容 *">{{ old('body') ? old('body') : $subImageBlock->body }}</textarea>
                        ...
                        <div class="row">
                            <div class="col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2">
                                <div class="form-group">
                                    ...
                                    <img id="image-preview" src="{{ asset('assets/images/all/' . $subImageBlock->image) }}" alt="Image Preview" style="width: 80px; margin: 10px 0 0 14px;">
                            ...
                            <div class="col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2">
                                <div class="form-group"><div class="info-image-hover-container">
                                    ...
                                    <input type="text" id="search-image-hover" placeholder="画像ホバーを選択してください *" class="md-input"
                                        onkeyup="filterImageHovers()" onclick="toggleDropdown()" value="{{ $subImageBlock->image_hover }}">
                                    <div id="image-hover-dropdown" class="dropdown-content">
                                        @foreach ($imageHovers as $imageHover)
                                            <div class="image-hover-dropdown-item {{ $subImageBlock->image_hover == $imageHover->name ? 'selected-icon' : '' }}" 
                                                onclick="selectImageHover('{{ $imageHover->id }}', '{{ $imageHover->name }}')">
                                                {{ $imageHover->name }}
                                            </div>
                                        @endforeach
                                    </div>
    ...
    <div id="modal-popup" class="white-bg all-padding-60 mfp-with-anim mfp-hide centerize-col col-lg-4 col-md-6 col-sm-7 col-xs-11 text-center">
        <span class="text-uppercase font-30px font-600 mb-20 display-block dark-color">サブ画像ブロック修正</span>
        <p class="mb-20">サブ画像ブロックを修正してもよろしいですか?</p>
    ...
    <script>
        function showModal() {
            if (validateEditSubimageBlockForm()) {
        ...
        function submitForm() {
            ...
            document.getElementById('edit-sub-image-block-form').submit();
        }

        function validateEditSubimageBlockForm() {
            let isValid = true;
